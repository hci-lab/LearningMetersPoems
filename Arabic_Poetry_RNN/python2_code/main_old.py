...
Experiement_Name = 'test_test'
earlystopping_patience=-1  
test_size_param=0.1
validation_split_param = 0.1
n_units = 10
input_data_path = "clean_data.csv"
...
input_arabic_poetry_dataset = pd.read_csv(input_data_path,encoding='utf-8',sep = ",")
...
if (old_date_flag == 1):
    ...
    input_arabic_poetry_dataset['Bayt_Text'] = input_arabic_poetry_dataset['Bayt_Text'].apply(lambda x: re.sub(r'(^'+our_alphabets+')','',str(x))).apply(lambda x: re.sub(r'  *'," ",x)).apply(lambda x: re.sub(u'ّ+', u'ّ', x)).apply(lambda x: x.strip())
    ...
    cols = [0,1,2,3,4,6,7]
    input_arabic_poetry_dataset.drop(input_arabic_poetry_dataset.columns[cols], axis=1,inplace=True)
    print input_arabic_poetry_dataset.columns
