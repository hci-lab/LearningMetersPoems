...
\end{tikzpicture}
\caption{\textit{Binary} encoding}
\label{fig:binary}  
\end{subfigure}
\caption{ \ref{fig:one-hot} Encoding the word \textarabic{مَرْحَبَا} 
using $n$-letter alphabet \textit{one-hot} encoding; the \textit{one-hot} is
applied to encode Arabic and English verses; the only difference is the vector
length is 181 in Arabic and is 28 is English (26 letter, a white-space character
and an apostrophe).  In figure \ref{fig:binary} the same word is encoded using
\textit{binary} encoding, where the vector length $n = \ceil*{\log_2 l}$,  $l \in
\{181, 28\}$ is the alphabet length (Arabic/English).
}
